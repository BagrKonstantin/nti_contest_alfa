...
class UI_Task2(QMainWindow, Ui_MainWindow2):
    ...
    def raspisanie(self):
        try:
            ...
            if self.user[33] == "ИкТ":
                msg.setText("""Понедельник\tВторник\tСреда\tЧетверг\tПятница\tСуббота                 
                                Физика\tИнформатика\tКриптография\tЭлектроника\tСет. Оборуд.\tСети след. Пок.
    310\t231\t306\t108\t209\t228
    Физ. Культура	Начерт. Геом.	Рус. Яз	Матем. Анализ	Лин. Алгебра	Сис-мы коммут.
    Спорт. Зал.	154	324	110	189	116
    Теор. эл-магн. полей					
    """)
            elif self.user[33] == "ЗСС":
                msg.setText("""Понедельник	Вторник	Среда	Четверг	Пятница	Суббота
Криптография	Начерт. Геом.	Физика	Физ. Культура	Матем. Анализ	Сис-мы коммут.
306	154	310	Спорт. Зал	110	116
Электроника	Рус. Яз	Информатика	Лин. Алгебра	Сет. Оборуд.	Сети след. Пок.
108	324	231	189	209	228
	Теор. эл-магн. полей				
	210				
""")
            elif self.user[33] == "ПМ":
                msg.setText("""Понедельник	Вторник	Среда	Четверг	Пятница	Суббота
Информатика	Физ.культура	Теор. Вероятн.	Рус. Яз	Лин. Алгебра	Матем. Анализ
231	Спорт. Зал	102	324	189	110
Физика	Криптография	Электроника	Основы БД	Теор. эл-магн. полей	Прикл. Стат. Анал.
310	306	108	301	210	204
Основы ООП	Мат. Стат.				
179	105				
""")
            elif self.user[33] == "ИВТ":
                msg.setText("""Понедельник	Вторник	Среда	Четверг	Пятница	Суббота
Основы ООП	Физика	Начерт. Геом.	Криптография	Рус. Яз	Лин. Алгебра
179	310	154	306	324	189
Информатика	Физ.культура	Теор. Вероятн.	Электроника	Матем. Анализ	Корп. Инф. Сис.
231	Спорт. Зал	102	108	110	316
Мат. Стат.					
105					
""")
            elif self.user[33] == "АУТС":
                msg.setText("""Понедельник	Вторник	Среда	Четверг	Пятница	Суббота
Физ.культура	Теор. Вероятн.	Информатика	Матем. Анализ	Электроника	Корп. Инф. Сис.
Спорт.зал	102	231	110	108	316
Основы ООП	Физика	Начерт. Геом.	Рус. Яз	Основы БД	Лин. Алгебра
179	310	154	324	301	189
		Мат. Стат.			
""")

            # msg.setInformativeText("Хотите сохранить информацию перед выходом?")

            ok_button = msg.addButton('Закрыть', QMessageBox.AcceptRole)
